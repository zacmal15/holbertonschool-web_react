...
class TerserPlugin {
  ...
  async optimize(compiler, compilation, assets, optimizeOptions) {
    ...
    const matchingMinimizers = (name, info) => {
      ...
      for (let i = 0; i < implementations.length; i++) {
        ...
        const filter = filters && typeof filters[i] === "function" ? filters[i] : impl.filter;
    ...
    const workerCapable = implementations.map(impl => typeof impl.supportsWorker === "undefined" || typeof impl.supportsWorker === "function" && impl.supportsWorker());
    const binaryCapable = implementations.map(impl => typeof impl.supportsBinary === "function" && impl.supportsBinary());
    ...
    if (needCreateWorker) {
      ...
      getWorker = () => {
        ...
        new Worker(require.resolve("./minify"), {
          ...
          enableWorkerThreads: implementations.every((impl, i) => !workerCapable[i] || typeof impl.supportsWorkerThreads === "undefined" || impl.supportsWorkerThreads() !== false)
        });
     ...
     */
    const run = (options, matched) => getWorker && matched.every(i => workerCapable[i]) ? getWorker().transform(getSerializeJavascript()(options)) : minify(options);
  ...
  generatorFor(compilation, resource) {
    ...
    if (named.length === 0) {
      ...
      if (!generator) {
        compilation.errors.push(TerserPlugin.buildError(new Error(`Error with '${resource}': no '${asked}' preset in \`generate\`, which defines ${named.map(one => `'${one.name}'`).join(", ")}.`), resource));
        return undefined;
      }
    }
  ...
  async generateAsset(compiler, compilation, cache, asset, generator) {
    ...
    let output = /** @type {{ code: Buffer, filename?: string, width?: number, height?: number, errors?: (Error | string)[], warnings?: (Error | string)[] } | undefined} */
    ...
    if (!output) {
      ...
      output = {
        code: typeof generated.code === "undefined" ? input : Buffer.isBuffer(generated.code) ? generated.code : Buffer.from(generated.code),
        filename: generated.filename,
        width: generated.width,
        height: generated.height,
        errors: (generated.errors || []).map(item => TerserPlugin.buildError(/** @type {Error | ErrorObject | string} */item, name)),
        warnings: (generated.warnings || []).map(item => TerserPlugin.buildWarning(item, name))
      };
    ...
    if (/\[(width|height)\]/i.test(generatedName)) {
      compilation.errors.push(TerserPlugin.buildError(new Error(`Error with '${name}': '${generator.filename}' asks for a size this generator does not report.`), name));
      return;
    }
  ...
  async generate(compiler, compilation, variesOn, result, module) {
    ...
    if (!output) {
      ...
      try {
        ...
        compilation.errors.push(TerserPlugin.buildError(/** @type {Error | ErrorObject | string} */error, resource));
        return result;
      }
      output = {
        ...
        errors: (minified.errors || []).map(item => TerserPlugin.buildError(/** @type {Error | ErrorObject | string} */item, resource)),
        warnings: (minified.warnings || []).map(item => TerserPlugin.buildWarning(item, resource))
      };
      await cacheItem.storePromise(output);
    }
    for (const error of (/** @type {Error[]} */output.errors || [])) {
    ...
    compiler.hooks.compilation.tap(pluginName, compilation => {
      ...
      if (moduleGenerator) {
        const generatorData = getSerializeJavascript()({
          generator: Array.isArray(moduleGenerator.implementation) ? moduleGenerator.implementation.map(getVersion) : getVersion(/** @type {BasicMinimizerImplementation<EXPECTED_ANY> & MinimizeFunctionHelpers} */
        ...
        try {
          ...
          compilation.errors.push(TerserPlugin.buildError(new Error(`The \`generate\` option needs a webpack whose \`NormalModule\` \`processResult\` hook can await (>= 5.111); this one cannot: ${ /** @type {Error} */error.message}`), pluginName));
        }
      }
